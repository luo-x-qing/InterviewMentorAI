...
| `/record` | RecordPage | 录音 + 上传 |
| `/report` | ReportPage | 报告详情 |
...
- 「录音」Tab：品牌标语 + 圆形麦克风按钮，点击跳转录音
- 「面试流程」Tab：5 步引导（自我介绍→技术能力→项目经验→情景分析→总结提问），STAR 法则
- 「评估报告」Tab：雷达图（CustomPainter 6 维度）+ 评分条 + 优势建议卡片
...
### 面试录音上传
```
录音页 → AudioService.startRecord() → 录音中 → stopRecord()
  → ApiService.uploadAudioFile() → POST /audio/upload
  → 后端异步分析 → 返回结果 → 跳转报告
```
...
| 主页 3 Tab | ✅ 完成 | 录音入口/面试流程/评估预览 |
| 个人中心 | ✅ 完成 | 用户信息 + 菜单 + 退出 |
| 录音 + 上传 | ✅ 完成 | 脉冲/波形动画 + 自动上传 |
| 报告展示 | ✅ 完成 | 雷达图 + 评分卡 + Markdown |
